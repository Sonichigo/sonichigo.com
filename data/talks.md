---
title: "Talks & Events"
description: "Speaking engagements, workshops, and events"
last_updated: "2026-09-25"
---

## Boosting DevRel: Simple SEO, Scalable Programs, and Community Power
- **Event:** DevRelKaigi 2025
- **Date:** 2025-10-04
- **Location:** Tokyo, Japan
- **Latitude:** 35.6762
- **Longitude:** 139.6503
- **Type:** speaker
- **Event URL:** [https://devrelkaigi.org/en/speakers/sonichigo](https://devrelkaigi.org/en/speakers/sonichigo)
- **Image URL:** /assets/img/data/reactplay-meetup-may.jpeg

A session focused on leveraging Azure OpenAI and Next.js to overcome language barriers, enabling more inclusive and globally accessible applications.

## Breaking Language Barriers with Azure OpenAI and Nextjs
- **Event:** ReactPlay Meetup May
- **Date:** 2025-05-17
- **Location:** Bengaluru, India
- **Latitude:** 12.9716
- **Longitude:** 77.5946
- **Type:** speaker
- **Event URL:** [https://www.meetup.com/reactplay-bengaluru/events/307690438/](https://www.meetup.com/reactplay-bengaluru/events/307690438/)
- **Image URL:** /assets/img/data/reactplay-meetup-may.jpeg

Explored practical implementation patterns for integrating AI-powered translation and multilingual support in modern web apps.

## Introduction to CNCF and CNCG Noida
- **Event:** CNCG Noida Meetup April
- **Date:** 2025-04-19
- **Location:** Noida, India
- **Latitude:** 28.5355
- **Longitude:** 77.3910
- **Type:** speaker
- **Event URL:** [https://www.linkedin.com/posts/sonichigo_kubeinloop-activity-7320314496596553731](https://www.linkedin.com/posts/sonichigo_kubeinloop-activity-7320314496596553731)
- **Image URL:** /assets/img/data/cncg-noida-april25.jpeg

An introductory session covering the Cloud Native Computing Foundation ecosystem and community initiatives like CNCG.

## Breaking Language Barriers with Azure OpenAI and Nextjs
- **Event:** React n Roll Meetup April
- **Date:** 2025-04-12
- **Location:** Bengaluru, India
- **Latitude:** 12.9716
- **Longitude:** 77.5946
- **Type:** speaker
- **Event URL:** [https://www.meetup.com/techinsider-bangalore/events/306842947/](https://www.meetup.com/techinsider-bangalore/events/306842947/)
- **Image URL:** /assets/img/data/reactnroll-april25.jpeg

Discussed building AI-first frontend applications with seamless multilingual capabilities.

## Testing in production with eBPF
- **Event:** CNCF Hooghly March 2025
- **Date:** 2025-04-08
- **Location:** Hooghly, India
- **Latitude:** 22.9068
- **Longitude:** 88.3882
- **Type:** speaker
- **Event URL:** [https://community.cncf.io/events/details/cncf-cloud-native-hooghly-presents-kubekulture/](https://community.cncf.io/events/details/cncf-cloud-native-hooghly-presents-kubekulture/)
- **Image URL:** /assets/img/data/cncf-hooghly-feb.jpeg

A deep dive into leveraging eBPF for safe and effective production testing, enhancing observability and debugging.

## Improving Developer Productivity with DevPod
- **Event:** CNCF Chennai Feb 2025
- **Date:** 2025-02-22
- **Location:** Chennai, India
- **Latitude:** 13.0827
- **Longitude:** 80.2707
- **Type:** speaker
- **Event URL:** [https://community.cncf.io/events/details/cncf-cloud-native-chennai-presents-cloud-native-chennai-feb-2025-meetup](https://community.cncf.io/events/details/cncf-cloud-native-chennai-presents-cloud-native-chennai-feb-2025-meetup)
- **Image URL:** /assets/img/data/cncf-chennai-feb-2025.jpeg

Highlighted how DevPod streamlines developer environments and accelerates onboarding and productivity.

## SEO for DevRel, and the Impact It Creates on the Business
- **Event:** DevRelCon India 2024
- **Date:** 2024-12-06
- **Location:** Bengaluru, India
- **Latitude:** 12.9716
- **Longitude:** 77.5946
- **Type:** speaker
- **Event URL:** [https://blr24.devrelcon.dev/](https://blr24.devrelcon.dev/)
- **Image URL:** /assets/img/data/devrelcon.jpeg

Focused on aligning Developer Relations with SEO strategies to drive measurable business impact.

## How to Generate Test-Cases and Data Mocks for Microservices at Kernel Using eBPF
- **Event:** Open Source Summit EU 2024
- **Date:** 2024-09-17
- **Location:** Vienna, Austria
- **Latitude:** 48.2082
- **Longitude:** 16.3738
- **Type:** speaker
- **Event URL:** [https://www.youtube.com/watch?v=VoV2eyTiFyY](https://www.youtube.com/watch?v=VoV2eyTiFyY)
- **Video URL:** [https://www.youtube.com/watch?v=VoV2eyTiFyY](https://www.youtube.com/watch?v=VoV2eyTiFyY)
- **Image URL:** /assets/img/data/oss-eu-2024.png

Demonstrated kernel-level testing techniques using eBPF to generate realistic test cases and mocks for microservices.

## Ship Features Faster with AI Based Testing Using Keploy
- **Event:** API World 2024
- **Date:** 2024-11-13
- **Location:** Virtual
- **Type:** speaker
- **Event URL:** [https://apiworld2024.sched.com/event/1kita/](https://apiworld2024.sched.com/event/1kita/)
- **Image URL:** /assets/img/data/apiworld-2024.png

Explored AI-driven testing workflows using Keploy to accelerate feature delivery cycles.

## Building Microservices with DevPod and Keploy
- **Event:** Platform Meetup July 2024
- **Date:** 2024-07-09
- **Location:** Bengaluru, India
- **Latitude:** 12.9716
- **Longitude:** 77.5946
- **Type:** speaker
- **Event URL:** [https://www.loft.sh/blog/platform-bengaluru-meetup-recap](https://www.loft.sh/blog/platform-bengaluru-meetup-recap)
- **Image URL:** /assets/img/data/june-platform-24.jpeg

Covered modern microservices development workflows combining DevPod environments and Keploy testing.

## Improving Developer Productivity with DevPod
- **Event:** GirlScript Summer of Code
- **Date:** 2024-06-26
- **Location:** Virtual
- **Type:** speaker
- **Event URL:** [https://www.youtube.com/watch?v=1DP6WDTsL8s](https://www.youtube.com/watch?v=1DP6WDTsL8s)
- **Video URL:** [https://www.youtube.com/watch?v=1DP6WDTsL8s](https://www.youtube.com/watch?v=1DP6WDTsL8s)
- **Image URL:** /assets/img/data/gssoc-june-24.jpg

An introductory session on improving developer workflows using DevPod in collaborative environments.

## Understanding eBPF Tracing
- **Event:** DevOpsDays Bangalore 2023
- **Date:** 2023-12-13
- **Location:** Bengaluru, India
- **Latitude:** 12.9716
- **Longitude:** 77.5946
- **Type:** speaker
- **Event URL:** [https://www.youtube.com/watch?v=Q7-fDN3ZOuc](https://www.youtube.com/watch?v=Q7-fDN3ZOuc)
- **Video URL:** [https://www.youtube.com/watch?v=Q7-fDN3ZOuc](https://www.youtube.com/watch?v=Q7-fDN3ZOuc)
- **Image URL:** /assets/img/data/devopsdays-23-blr.png

Explained eBPF tracing fundamentals and its applications in observability and debugging.

## Infuse Security Into your SDLC
- **Event:** DevSecCon 2022
- **Date:** 2022-06-15
- **Location:** Virtual
- **Type:** speaker
- **Event URL:** [https://www.youtube.com/watch?v=SiDQJqRsUDE](https://www.youtube.com/watch?v=SiDQJqRsUDE)
- **Video URL:** [https://www.youtube.com/watch?v=SiDQJqRsUDE](https://www.youtube.com/watch?v=SiDQJqRsUDE)
- **Image URL:** /assets/img/data/devseccon-22.jpeg

Focused on integrating security practices directly into the software development lifecycle.

## Generation Test-Cases and Mocks from Production
- **Event:** PuneFOSS 2023
- **Date:** 2023-04-01
- **Location:** Pune, India
- **Latitude:** 18.5204
- **Longitude:** 73.8567
- **Type:** speaker
- **Image URL:** https://pbs.twimg.com/media/FsE6PWFagAIn8ho?format=jpg&name=medium

Demonstrated how to leverage production traffic to generate realistic test cases and mocks for microservices using eBPF.

## Generating Test Cases and Mocks from Production
- **Event:** FOSS Kolkata 2023
- **Date:** 2023-02-27
- **Location:** Kolkata, India
- **Latitude:** 22.5726
- **Longitude:** 88.3639
- **Type:** speaker
- **Image URL:** https://pbs.twimg.com/media/FpyxG96XoAEgGH3?format=jpg&name=medium

Demonstrated how to leverage production traffic to generate realistic test cases and mocks for microservices using eBPF.

## Securing the application in SDLC
- **Event:** DevSecCon 2023
- **Date:** 2023-06-23
- **Location:** Virtual
- **Type:** speaker
- **Video URL:** [https://www.youtube.com/watch?v=TuwDqhE9r4s](https://www.youtube.com/watch?v=TuwDqhE9r4s)
- **Image URL:** /assets/img/data/devseccon-23.jpg

In today's digital world, security breaches and data thefts are becoming more and more common. As a result, secure coding practices have become a top priority for software developers. However, with the increased reliance on software, comes the increased risk of security breaches and data thefts. It's crucial for developers to follow secure coding practices to mitigate these risks.

## Introduction to Kubernetes - Virtual Session
- **Event:** Cloud Dev Day 2022
- **Date:** 2022-06-12
- **Location:** Noida, India
- **Latitude:** 28.5355
- **Longitude:** 77.3910
- **Type:** speaker
- **Image URL:** /assets/img/data/cloud-dev-day-22.jpg

An introductory session on Kubernetes fundamentals, architecture, and ecosystem for developers new to container orchestration.

## Introduction to eBPF : Networking, Monitoring & Tracing
- **Event:** Cloud Native Noida Meetup Feb 2024
- **Date:** 2024-02-09
- **Location:** Virtual
- **Type:** speaker

An introductory session on eBPF fundamentals, use cases in networking, monitoring, and tracing for cloud-native applications.

## Introduction to CNCF and CNCG Noida
- **Event:** cTENCF Meetup August 2025
- **Date:** 2025-08-23
- **Location:** Noida, India
- **Latitude:** 28.5355
- **Longitude:** 77.3910
- **Type:** speaker
- **Image URL:** /assets/img/data/ctenf-meetup-25.jpg

An introductory session covering the Cloud Native Computing Foundation ecosystem and community initiatives like CNCG.

## Introduction to CNCF and CNCG Noida
- **Event:** CNCG Noida Meetup March 2026
- **Date:** 2026-03-21
- **Location:** Delhi, India
- **Latitude:** 28.7041
- **Longitude:** 77.1025
- **Type:** speaker
- **Image URL:** /assets/img/data/cncg-noida-march-26.jpeg

An introductory session covering the Cloud Native Computing Foundation ecosystem and community initiatives like CNCG.

## Stop Fighting MongoDB: Native Database Migrations Without the Compatibility Headaches
- **Event:** MongoDB Meetup May 2026
- **Date:** 2026-05-16
- **Location:** Harness, Bengaluru, India
- **Latitude:** 12.9716
- **Longitude:** 77.5946
- **Type:** speaker
- **Image URL:** /assets/img/data/mongodb-harness-26.jpeg

The talk will cover how teams can take a more native approach to MongoDB migrations, working directly with the database instead of relying on compatibility layers. It will touch on what happens when tools like Liquibase don’t fully align with MongoDB and how teams navigate those gaps. A live demo of a Java microservice will show how migrations can run cleanly from pipeline to database in a real-world setup

## Undestanding IoT: Introduction, Applications, and Future Implications
- **Event:** Workshop at UVEC 2024
- **Date:** 2024-07-27
- **Location:** Bengaluru, India
- **Latitude:** 12.9716
- **Longitude:** 77.5946
- **Type:** workshop
- **Image URL:** /assets/img/data/uvce-workshop-24.png

An introductory workshop on the fundamentals of the Internet of Things (IoT), exploring its applications across various industries and discussing future implications for technology and society.

## Skills Are Not MCP Servers: When To Use Which (and How To Make Them Work Together)
- **Event:** MCP Dev Summit 2026
- **Date:** 2026-06-10
- **Location:** Bengaluru, India
- **Latitude:** 12.9716
- **Longitude:** 77.5946
- **Type:** speaker
- **Video URL:** [https://youtu.be/_YYID1F9Jsw?si=CkKFJEDdUqSJZ78-](https://youtu.be/_YYID1F9Jsw?si=CkKFJEDdUqSJZ78-)


In this talk I'll break down where the boundary actually is. Skills are great when the knowledge is stable conventions, workflow logic, best practices. MCP servers earn their keep when you need live data, real-time API calls, or actions with side effects.

## From SSE To Streamable HTTP: What Actually Changed in MCP's Transport Layer and Why You Should Care
- **Event:** MCP Dev Summit 2026
- **Date:** 2026-06-09
- **Location:** Bengaluru, India
- **Latitude:** 12.9716
- **Longitude:** 77.5946
- **Type:** speaker
- **Video URL:** [https://youtu.be/hOmhM0oZPa8?si=Uzk7TgVgaJIicBA9](https://youtu.be/hOmhM0oZPa8?si=Uzk7TgVgaJIicBA9)

In this talk, I'll walk through MCP's transport journey, stdio for local tooling, the dual-endpoint SSE model that powered early remote servers, and the Streamable HTTP design that replaced it in the March 2025 spec update.

## Why Agents Make Different Decisions With the Same Tools.
- **Event:** MCP Dev Summit 2026
- **Date:** 2026-06-10
- **Location:** Bengaluru, India
- **Latitude:** 12.9716
- **Longitude:** 77.5946
- **Type:** speaker
- **Video URL:** [https://youtu.be/lSjakfdOUqc?si=272IF6CySPzOXIDu](https://youtu.be/lSjakfdOUqc?si=272IF6CySPzOXIDu)

Scenario: Deploy an agent to production. Works 90% of time in testing. Month later: Claude model updates. Success rate drops to 70%. Why? Model change altered how tools are ranked.
You can't see this. You have no control. Your agent silently degraded. This talk identifies sources of divergence.

## What happens when AI meets OPA
- **Event:** OSS South Korea 2026
- **Date:** 2026-08-11
- **Location:** Seoul, South Korea
- **Latitude:** 37.5665
- **Longitude:** 126.9780
- **Type:** speaker
- **Video URL:** [https://youtu.be/sXYI3o-rOaM?si=WWFdzioYGNBlDfep](https://youtu.be/sXYI3o-rOaM?si=WWFdzioYGNBlDfep)

Scenario: Tom and Jerry has run for over 80 years. Every episode follows the same plot: Tom builds an elaborate trap, Jerry walks straight through it, the house is destroyed, and the owner blames Tom. Sound familiar? You can't see this. You have no control. Your agent silently degraded.

## Your MCP Server Behaves Differently in Every Client: A Cross-Client Field Guide for Server Authors
- **Event:** MCP Dev Summit 2026
- **Date:** 2026-08-14
- **Location:** Seoul, South Korea
- **Latitude:** 37.5665
- **Longitude:** 126.9780
- **Type:** speaker
- **Video URL:** [https://youtu.be/FUNC3-MxMDQ?si=H6U01r6AyitFBxPI](https://youtu.be/FUNC3-MxMDQ?si=H6U01r6AyitFBxPI)

Scenario: Same MCP server. Same prompt. Three different IDEs. Three different outcomes — and one of them is silently wrong.  After documenting a production MCP server across Cursor, Claude Desktop, and Windsurf, the speaker has the war stories.

## The Blast Radius Problem: Why Database MCP Servers Need Different Safety Primitives
- **Event:** ContainerDays & AI Context 2026
- **Date:** 2026-10-28
- **Location:** Singapore, Singapore
- **Latitude:** 1.3521
- **Longitude:** 103.8198
- **Type:** speaker

Scenario: You have a database MCP server. You have a client. You have a prompt. You have a model. You have a user. You have a bug. You have a blast radius. This talk will explore the safety primitives that database MCP servers need to prevent catastrophic failures in production environments.
